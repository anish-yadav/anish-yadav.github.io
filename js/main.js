...
function changeBG() {
    ...
    if (Math.abs(homefromTop) < home.clientHeight / 2) {
        ...
        document.body.classList.remove('bg-dark', 'bg-2', 'bg-1', 'bg-3')
        ...
        document.body.classList.remove('bg-1', 'bg-dark', 'bg-2', 'bg-3')
        document.body.classList.add('bg-1')
        showNavbar('bg-1')
...
const scrollToWork = () => {
    window.scrollTo({
        top: work1.offsetTop - document.documentElement.scrollTop,
        behavior: 'smooth'
    })
}
